"""Training script with register tokens.

Based on "Vision Transformers Need Registers" (Darcet et al., 2024), register tokens
provide a "scratch space" for attention, allowing the model to dump information that
doesn't need to attend to meaningful input tokens.

This script prepends N register tokens to each sequence before the input digits.
"""
import torch
...
PAD_TOKEN = 10
EOS_TOKEN = 11
REG_TOKEN = 12  # Register token
VOCAB_SIZE = 13  # 0-9 + PAD + EOS + REG
...
def generate_batch(batch_size, num_digits, num_registers, device):
    """Generate a batch of add-7 examples with register tokens.

    Format: <REG>...<REG><reversed_input><EOS><reversed_output><EOS>
    """
    ...
    for i in range(batch_size):
        ...
        # Sequence: registers + input_digits + EOS + output_digits + EOS
        seq = [REG_TOKEN] * num_registers + in_digits + [EOS_TOKEN] + out_digits + [EOS_TOKEN]
        sequences.append(seq)

    # Stack into tensor
    sequences = torch.tensor(sequences, dtype=torch.long, device=device)
    return sequences, num_registers + num_digits  # return prefix length for masking


def compute_loss(model, sequences, prefix_len):
    ...
    # The first EOS is at position prefix_len, so we predict from prefix_len onwards
...
def evaluate(model, num_digits, num_registers, device, num_samples=256):
    ...
    with torch.no_grad():
        for _ in range(num_samples):
            ...
            # Build input sequence: registers + input + EOS
            seq = torch.tensor(
                [[REG_TOKEN] * num_registers + in_digits + [EOS_TOKEN]],
                dtype=torch.long,
                device=device
            )
...
def main():
    parser = argparse.ArgumentParser(description='Train add-7 transformer with register tokens')
    parser.add_argument('--num_digits', type=int, default=2)
    parser.add_argument('--num_registers', type=int, default=1, help='Number of register tokens to prepend')
    ...
    print(f"Using {args.num_registers} register token(s)")

    # Initialize wandb
    if not args.no_wandb:
        wandb.init(project=args.wandb_project, config=vars(args))

    # Create model with expanded vocab for register token
    ...
    for step in range(1, args.steps + 1):
        sequences, prefix_len = generate_batch(
            args.batch_size, args.num_digits, args.num_registers, device
        )

        optimizer.zero_grad()
        loss = compute_loss(model, sequences, prefix_len)
        loss.backward()
        ...
        if step % 10 == 0:
            ...
            if step % args.eval_interval == 0:
                acc = evaluate(model, args.num_digits, args.num_registers, device)
                ...
                if acc > best_acc:
                    ...
                    patience_counter = 0
                    # Save best model
                    ckpt_path = ckpt_dir / 'best_model_registers.pt'
                    torch.save({
                        'step': step,
    ...
    # Final evaluation
    final_acc = evaluate(model, args.num_digits, args.num_registers, device, num_samples=1000)
